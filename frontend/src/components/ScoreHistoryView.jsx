...
const ScoreHistoryView = () => {
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.text(`Name: ${user?.name || 'Student'}`, 15, 63);
      doc.text(`School: ${user?.schoolName || 'Neo Karma Academy'}`, 15, 70);
      doc.text(`Grade: ${user?.grade || 10} (${user?.locationType || 'urban'})`, 15, 77);
      doc.text(`Generated: ${new Date().toLocaleDateString()}`, 148, 63);
      ...
      metricCards.forEach((card, index) => {
        ...
        doc.roundedRect(x, 86, 40, 24, 2.5, 2.5, 'F');
        doc.setDrawColor(226, 232, 240);
        doc.roundedRect(x, 86, 40, 24, 2.5, 2.5, 'D');
        doc.setTextColor(100, 116, 139);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(7);
        doc.text(card.label.toUpperCase(), x + 4, 94);
        doc.setTextColor(10, 61, 37);
        doc.setFontSize(13);
        doc.text(card.value, x + 4, 104);
      });
      ...
      // Be specific about the data timeframe and explicitly show it is uneven & incomplete
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8.5);
      doc.setTextColor(100, 116, 139);
      let timeframeText = 'No logs recorded';
      if (rowsToPrint.length > 0) {
        const oldestDate = formatDateString(rowsToPrint[rowsToPrint.length - 1].date);
        const newestDate = formatDateString(rowsToPrint[0].date);
        timeframeText = `Timeframe: ${oldestDate} - ${newestDate} (${rowsToPrint.length} recorded entries )`;
      }
      doc.text(timeframeText, 15, currentY + 5.5);
      ...
      doc.text('DAILY CARBON FOOTPRINT', 80, currentY + 5.5);
      doc.text('PRACTICAL SCORE EARNED', 150, currentY + 5.5);
      ...
      // Draw footer divider line to prevent any overlap issues
      doc.setDrawColor(226, 232, 240);
      ...
      doc.text('Generated by Neo Karma.', 15, 285);
      doc.text(`Page 1 of 1`, 195 - doc.getTextWidth('Page 1 of 1'), 285);
